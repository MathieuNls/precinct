...
This said, only a fraction of the code is analyzed, making PRECINCT efficient compared to leading clone detection techniques such as NICAD (Accurate Detection of Near-miss Intentional Clones) [@Cordy2011].
...
In this study, we focus on Type 3 clones as they are more challenging to detect. Since Type 3 clones include Type 1 and two clones, then these types could be detected separately by PRECINCT as well.
...
Text-based techniques use the code — often raw (e.g. with comments) — and compare sequences of code (blocks) to each other to identify potential clones. Johnson was perhaps the first one to use fingerprints to detect clones [@Johnson1993; @Johnson1994]. Blocks of code are hashed, producing fingerprints that can be compared.
...
Another approach to detecting clones is to use static analysis and to leverage the semantics of the program to improve the detection.
...
Niko _el al._ proposed techniques revolving around hashing to get a quick answer while detecting type-1, type-2, and type-3 clones in Squeaksource [@Niko2012]. 
While their approach works on a single-system (i.e. detecting clones on one version of one system), they found that more than 14% of all clones are copied from project to project; stressing even more the need for fast and scalable approaches for clone detection as the code to analyze grow exponentially with new versions and project added on a daily basis.
...
While these results are promising, especially because the approach is detecting clones on cross-projects and cross-versions environment, the computing power required is still considerable. 
In comparison, four years later, we used 4 Core @ 2.5GHz with 8 GB of RAM; which we consider to be a common developer configuration and reach sub-second performances.

Similarly, Saini _et al_ and Sajnani _et al_ proposed approaches named SourcererCC and its related eclipse plugin SourcererCC-I [@Saini2016; @Sajnani2016].
SourcererCC targets fast clone detection on developers' workstation (12 GB RAM). 
They achieve a precision of 86% and recall 86%-100% for clones of type 1, 2 and 3 while testing on BigCloneBench [@svajlenko2014towards].
SourcererCC is a token-based clone detector that uses an optimized inverted-index, and it has been exercised on 25K projects cumulating 250 MLOC.

Toomey _el al._ also proposed an efficient token based approach for detecting clones called ctcompare [@Toomey2012].
Their tokenization is, however, different than most approaches as they used lexical analysis to produce sequences of token that can be transformed into token tuples.
ctcompare is accurate, scalable and fast but lacks type 3 clone detection that arguably more challenging to detect.

While faster algorithms are proposed, and the workstations of developers get more powerful, the exponential growth source code base makes the detection of clones a time taxing task for developers.
...
Incremental clone detection is a technique where only the changes from a version to another are analyzed.
...
While being extremely fast and reliable, Hummel _et al._ need an industrial cluster to achieve such performances (the fourth author works at Google).
In our opinion, it is unlikely that standard practitioners have access to such computational power.
Moreover, they only target type 1 and 2.
Higo _et al._ proposed an incremental clone detection approach based on program dependency graphs (PDG) [@Higo2011]. 
Using PDG is arguably more complex than text comparison and allows the detection of clones structure that scattered in the program.
They were able to analyze 5,903 revisions in 15 hours in Apache Ant.  

The techniques above, however, focus on detecting clones after they are inserted in code. 
Only a few studies focus on preventing the insertion of clones. Lague et al. [@Lague] conducted a very large empirical study with 10,000 developers over 3 years, where developers where asked to use clone detection tools during the development process of a very large telecom system. 
The authors found that while clones are being removed over time, using clone detection tools help to improve the quality of the system as it prevents defects to reach the customers. 
Duala et al. [@Duala-Ekoko2007; @Duala-Ekoko2010] proposed to create clone region descriptors (CRDs), which describe clone regions within methods in a robust way that is independent of the exact text of the clone region or its location in a file. 
Then, using CRDs, clone insertion can be prevented.

PRECINCT aims to prevent clone insertion while integrating the clone detection process in a transparent manner in the day-to-day development process. 
This way, software developers do not have to resort to external tools to remove clones after they are inserted. Our approach operates at commit time, notifying software developers of possible clones as they commit their code.
...
Zibran recently proposed an infrastructure for integrated clone management that covers clone detection and refactoring on developers' workstations supported by a remote server infrastructure [@Zibran2016].
It could, if implemented, fit in the category of "Just-In-Time Clone Detection."
This work is close to our in a sense that they propose and incremental clone detection that is implementable in the day-to-day workflow of developers.
We argue, however, that our approach is simpler to implement and uses directly the code versioning tool rather than yet another plugin in developer's IDE. 
Finally, our approach has been implemented and tested while Zibran _et al._ only theorized a framework to be tested.
...
Second, NICAD can detect all Types 1, 2 and 3 software clones.
...
The aim of the case study is to answer the following question: *Can we detect clones at commit time, i.e., before they are inserted into the final code, if so, what would be the accuracy compared to a traditional clone detection tool such as NICAD?*
...
We assess the performance of PRECINCT in terms of precision and recall.
Both precision and recall are computed by considering NICAD’s results as a baseline.
We also compute F$_{1}$-measure, i.e., the weighted average of precision and recall, to measure the accuracy of PRECINCT.
...
To the best of our knowledge, we are the first to propose an incremental clone detection method that fits directly into the versioning process of source code.
In this section, we discuss the advantages and shortcomings of our approach when compared to other well-known techniques targeting clone detection.

Research in clone detection has evolved over the years to include faster algorithms [@Niko2012; @Saini2016; @Sajnani2016], incremental [@Higo2011] and scalable [@Hummel2010] approaches.
Nonetheless, the literature shows that software maintenance tools have yet to be adopted by industry [@Lewis2013; @Foss2015; @Layman2007; @Ayewah2007; @Ayewah2008; @Johnson2013; @Norman2013; @Hovemeyer2004; @Lopez2011].
This fact is not limited to clone detection particularly, but to any tools or approaches aiming to ease the software maintenance activity of developers.

In the case of clone detection, there is two classes of approaches: external tools [@Kamiya2002; @Roy2008; @Duala-Ekoko2007; @Jiang2007; @Lee2010] and IDE plugins [@Saini2016; @Sajnani2016; @VanTonder2016; @Lee2010]. 
We hypothesize that both classes are equally flawed when it comes to industrial adoption.

-  External tools, on one hand, lack of integration with developers' workflow (i.e., coding, testing,  debugging, committing) and using them lead to a significant overhead linked to context-switching [@Robertson2004; @Robertson2006; @Beckwith2006]. 
Moreover, for each tool, developers have to download, install and understand it. 
-  IDE plugins, on the other hand, interrupt developers during their coding sessions and make them lost track of parts of their mental model [@LaToza2006; @Ko2006d]. 
Moreover, Latoza _et al_ found that it exists six different reasons to duplicate code ((a) separate developers implement same functionality, (b) copy and paste of example code, (c) design decision distributed over multiple methods, (d) copy of other team’s code base, (e) branch maintained separately, (f) reimplementation by same developer in different language) and developers are aware that they are creating clones in five out of the six situations (b, c, d, e, and f) [@LaToza2006].

External tools and IDE plugins create overhead either by context switching or interrupting the developers to tell them something they likely know.
PRECINCT is different.
With our approach, we acknowledge the fact that developing software is a mentally challenging task and developers mental model should not be tempered with.
Consequently, we integrate ourselves in the most transparent manner: at commit time.
...
It is, therefore, natural to remind developers about a clone they purposely introduced or, in the case of separate developers implementing same functionality; warn them, at commit-time.
To summarize, PRECINCT does not require the context-switching and learning curve of an external tool nor interrupt developers during their coding session.
